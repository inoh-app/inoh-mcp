...
/** The account every spec here drives. */
export const TEST_ACCOUNT_EMAIL = 'e2e-mcp@test.com';
...
export type SeededAccount = {
  ...
  dueWords: string[];
  spareWords: string[];
  /** Words seeded into the account's own private dictionary, newest first. */
  privateWords: string[];
};
 ...
 *
 * @param options - The account's address, plan, seed profile, and how many
 *   cards to put in its own private dictionary
...
export const readAccountState = (email: string): AccountState =>
  _runFixtureCommand<AccountState>('state', { email });
